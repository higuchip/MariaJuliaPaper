...
# Configure theme for scientific publication
...
# If Urubici not found, use buffer around points
if(nrow(urubici) == 0) {
  ...
  urubici <- st_buffer(urubici_center, dist = 0.15) # ~15km radius
...
panel_b <- ggplot() +
  ...
  # Add label for Urubici
  annotate("text", x = -49.5, y = -28.0, label = "Urubici", 
           size = 3.5, fontface = "bold") +
  
  # Add label for Santa Catarina
...
# ===== PANEL C: URUBICI DETAIL WITH AREAS =====
print("Creating Panel C - Urubici Detail...")

# Calculate bbox for panel C based on Urubici and points
if(class(urubici)[1] == "sf") {
  bbox_urubici <- st_bbox(urubici)
  bbox_points <- st_bbox(study_areas_sf)
  
  # Use the larger bbox to ensure everything appears
...
panel_c <- ggplot() +
  ...
  # Add Urubici label inside the municipality
  annotate("text", 
...
# ===== CREATE BALANCED LAYOUT VERSION =====
print("Creating balanced proportions version...")

# Alternative layout with more balanced heights
mapa_alternativo <- (panel_a | panel_b) / panel_c +
  plot_layout(heights = c(1, 1.2)) +  # Panel C only 20% larger
  plot_annotation(
...
# Save as Fig1.tiff for publication
ggsave(
  filename = "Fig1.tiff",
  plot = mapa_alternativo,
  width = 180,
  height = 190,
  units = "mm",
  dpi = 300,
...
print("File saved: Fig1.tiff")

# ===== SUPPLEMENTARY INFORMATION =====
# Create coordinate table for manuscript
...
# Save table
write.csv(coord_table, "study_areas_coordinates.csv", row.names = FALSE)