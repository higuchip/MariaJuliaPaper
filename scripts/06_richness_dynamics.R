...
# ==============================================================================
#
# Description: Analyzes temporal changes in species richness across abandoned
#              pastures and natural grassland using GAMMs with temporal
#              autocorrelation correction (CAR1).
#
...
#
# Output:      ../artigo/biotropica/review/Figure_5_richness.tiff
#              ../artigo/biotropica/review/Figure_S3_richness_diagnostics.tiff
#              outputs/tables/Richness_Results_summary.csv
#
...
# Calculate richness (unique species per Area x Year)
...
primeiro_ano <- min(richness_data$Ano)
ultimo_ano <- max(richness_data$Ano)
...
gamm_richness_base <- gamm(
  Riqueza ~ s(Ano, k = k_antrop),
...
aic_base <- AIC(gamm_richness_base$lme)
aic_cor  <- AIC(gamm_richness_cor$lme)
...
if(delta_aic > 2) {
  ...
  best_model_r <- gamm_richness_cor
  used_cor <- TRUE
} else {
  cat("-> CONCLUSION: CAR1 not significantly better. Using baseline.\n")
  best_model_r <- gamm_richness_base
  used_cor <- FALSE
}

# --- D. Heteroscedasticity Check ---
res_temp <- resid(best_model_r$lme, type = "normalized")
fit_temp <- fitted(best_model_r$lme)
...
final_summary <- summary(final_model_r$gam)
final_smooth <- final_summary$s.table["s(Ano)",]
residuos_final <- resid(final_model_r$lme, type = "normalized")

# Phi parameter (temporal autocorrelation)
phi_value <- NA
if(used_cor) {
  cs <- final_model_r$lme$modelStruct$corStruct
...
pred_inicial <- predict(final_model_r$gam, 
                        newdata = data.frame(Ano = primeiro_ano, Area_Antrop = "1"))
pred_final <- predict(final_model_r$gam, 
...
fit_final <- fitted(final_model_r$lme)
...
cat("\n=== DETAILED RESULTS (RICHNESS) ===\n")

results_simple_richness <- data.frame(
  Parameter = c(
    "Temporal trend (F)",
    "p-value",
    "Trend type (EDF)",
    "Adjusted R²",
    "Rate of change (sp/year)",
    "Total increment (%)",
    "Baseline Model (AIC)",
    "CAR1 Model (AIC)",
    "Delta AIC",
  ...
  Value = c(
    ...
    status_acf,
    paste0(round(slope_ref, 1), " ± ", round(se_slope_ref, 1))
  ),
...
print(results_simple_richness, row.names = FALSE)
write.csv(results_simple_richness, "outputs/tables/Richness_Results_summary.csv", row.names = FALSE)
...
# Prediction grid
anos_pred <- seq(min(antrop_richness_data$Ano), max(antrop_richness_data$Ano), length.out = 100)

# Anthropogenic predictions
pred_antrop <- data.frame(Ano = anos_pred, Area_Antrop = "1")
pred_antrop_list <- predict(final_model_r$gam, newdata = pred_antrop, se.fit = TRUE)
...
pred_ref_list <- predict(ref_gam, newdata = pred_ref, se.fit = TRUE)
...
ggsave(paste0(output_path, "Figure_5_richness.tiff"), fig_main_richness,
       width = 14, height = 10, units = "cm", dpi = 300, compression = "lzw")
cat("Main figure saved:", paste0(output_path, "Figure_5_richness.tiff"), "\n")
...
residuos <- resid(final_model_r$lme, type = "normalized")
valores_ajustados <- fitted(final_model_r$lme)

tiff(paste0(output_path, "Figure_S3_richness_diagnostics.tiff"),
...
dev.off()
cat("Diagnostic figure saved:", paste0(output_path, "Figure_S3_richness_diagnostics.tiff"), "\n")
